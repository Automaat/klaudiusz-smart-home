...
This document contains instructions for configuration that cannot be done declaratively through NixOS and must be performed manually through the Home Assistant UI.

## MQTT Broker Configuration

**Why manual?** Since Home Assistant 2022.3, MQTT broker connection settings can no longer be configured via YAML. The integration must be set up through the UI.
